...
local function dropFood(bodyPos)
	...
	local dropPos = bodyPos + Vector3.new(
		math.random(-6, 6),
		-8,
		math.random(-6, 6)
	)
...
local function runFlight()
	if airplaneState then return end  -- only one airplane at a time
	...
	local zOptions = {}
	for z = -480, -200, 30 do table.insert(zOptions, z) end
	for z =  200,  480, 30 do table.insert(zOptions, z) end
...
local function schedulerLoop()
	while true do
		local waitTime = math.random(
			Config.AIRPLANE_INTERVAL_MIN,
			Config.AIRPLANE_INTERVAL_MAX
		)
		task.wait(waitTime)
